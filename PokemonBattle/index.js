...
class Pokemon {
  ...
  logPokemon() {
    console.log(...data:`Pokemon : ${this.name} ${this.emoji} ${this.health}
    Health : ${this.health}
    Attacks:`);
    this.logAttacks()
  ...
  getHealthString() {
    ...
    for(let i: number = 0; i++){
      health += copyHealth > 0 ? "-VERT-" : "-ROUGE-";
...
class Attack {
  ...
  performAttack() {
    const stabilityPowerRemove = Math.floor(Math.random() * this.power * (1 - this.stability))
    return this.power - stabilityPowerRemove;
  }

  logAttack() {
    console.log(
      ...data: `${this.name} -Power: ${this.power} - Stability: ${this.stability}`
    );
  }
}
...
const bulbasaur = new Pokemon('Bulbasaur', 1, [
...
const charmander = new Pokemon('charmander', 1, [
  new Attack('Flamethrower', 35, 0.1),
...
class Game{
  ...
  start(){
    ...
    this.opponentPokemon = Game.POKEMONS.filter(pokemon => pokemon !== this.playerPokemon)[
  ...
  async battle(){
    ...
    const oppponentAttack = this.opponentPokemon.randomAttack();
    const opponentAttackDamage = oppponentAttack.performAttack();

    console.log(
      `Play Attack use ${playerAttack.name} and made ${playerAttackDamage}.`
    );
  ...
  finish(){
    if(this.playerPokemon.health <= 0){
      console.log(
        `${this.playerPokemon.name} is out of health.`
      );
    } else {
      console.log(
        `${this.opponentPokemon.name} is out of health.`
      );
    }
  }
